...
1. Expose at least two MCP tools
1. Implement basic resilience:
1. Graceful errors for HTTP failures, timeouts, and empty results.
1. Respect API rate limits (e.g., simple backoff or user-facing warning).
1. Packaging and docs:
1. Provide clear setup instructions, environment variables, and run commands.
1. Include an example invocation flow (what to type/click in the client to trigger the tools).
1. Choose one deployment mode:
1. Local: STDIO server, runnable from your machine and discoverable by Claude Desktop or an AI IDE like Cursor.
1. Remote: HTTP server accessible over the network, callable by an MCP-aware client or an agent runtime. Extra credit if deployed and reachable.
1. (Optional) Bonus: Authentication
1. API key support via environment variable and client configuration; or
1. OAuth2-style bearer tokens for HTTP transport, validating token audience and never passing tokens through to upstream APIs.

1. 选择一个外部 API，并记录你将使用哪些端点。示例：天气、GitHub issues、Notion 页面、影视数据库、日历、任务管理器、金融/加密货币、旅行、体育统计。
1. 至少暴露两个 MCP 工具
1. 实现基本的韧性：
1. 对 HTTP 失败、超时和空结果给出优雅的错误。
1. 遵守 API 速率限制（例如简单的退避，或向用户提示）。
1. 打包与文档：
1. 提供清晰的搭建说明、环境变量和运行命令。
1. 包含一个调用流程示例（在客户端中输入什么或点击什么来触发这些工具）。
1. 选择一种部署模式：
1. 本地：STDIO 服务器，可在你的机器上运行，并能被 Claude Desktop 或 Cursor 这类 AI IDE 发现。
1. 远程：可通过网络访问的 HTTP 服务器，能被感知 MCP 的客户端或智能体运行时调用。若能部署并可访问，可获得额外加分。
1. （可选）加分项：认证
1. 通过环境变量与客户端配置支持 API 密钥；或
1. 为 HTTP 传输支持 OAuth2 风格的持有者令牌（bearer token），校验令牌受众，绝不把令牌透传给上游 API。
