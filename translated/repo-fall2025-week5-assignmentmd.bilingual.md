...
1. Run the app (from `week5/` directory)
...
Keep your changes focused on backend, frontend, logic, or tests inside `week5/`. For each selected task, note its difficulty level.

让你的改动集中在 `week5/` 内的后端、前端、逻辑或测试上。 对每个选定的任务，注明其难度等级。
...
1. Warp Drive workflows/rules (share links and/or exported definitions) and any helper scripts
1. Any supplemental prompts/playbooks used to coordinate multiple agents

1. 两个或更多 Warp 自动化，其中可包括：
1. Warp Drive 工作流/规则（分享链接和/或导出的定义）以及任何辅助脚本
1. 用于协调多个智能体的任何补充提示词/操作手册

1. A write‑up `writeup.md` under `week5/` that includes:
...
1. Autonomy levels used for each completed task (which code permissions, why, and how you supervised)
1. (if applicable) Multi‑agent notes: roles, coordination strategy, and concurrency wins/risks/failures
1. How you used the automation (what pain point it resolves or accelerates)

1. 一份放在 `week5/` 下的书面报告 `writeup.md`，其中包含：
1. 每项自动化的设计，包括目标、输入/输出、步骤
1. 前后对比（即人工工作流 vs. 自动化工作流）
1. 每个已完成任务所使用的自主性级别（使用了哪些代码权限、为什么，以及你如何监督）
1. （如适用）多智能体说明：角色、协调策略，以及并发的收益/风险/失败
1. 你如何使用该自动化（它解决了或加速了哪个痛点）
