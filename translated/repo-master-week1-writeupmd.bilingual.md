...
**c. Plans and task state**: `TODO: label` · evidence: `TODO` \ How does one get created and advanced? What does the model see about task state each turn, and where does it live in the request:

**c. 计划与任务状态**：`TODO: label` · 证据：`TODO` \ 计划是如何创建并推进的？模型每一轮能看到哪些关于任务状态的信息，这些信息又位于请求中的什么位置：

> TODO

> TODO

**d. Subagents**: `TODO: label` · evidence: `TODO` \ When the agent delegates, what the subagent is told, and what comes back:

**d. 子智能体**：`TODO: label` · 证据：`TODO` \ 当智能体委派任务时，子智能体会被告知什么，以及会返回什么：

> TODO

> TODO

**e. Context management**: `TODO: label` · evidence: `TODO` \ What changed in the payloads as the session grew:

**e. 上下文管理**：`TODO: label` · 证据：`TODO` \ 随着会话增长，请求负载中发生了什么变化：
...
1. Confirm no credentials or `x-api-key` headers made it into your quoted excerpts.
1. Push all changes to your remote repository and submit via Gradescope.
1. Don't forget to remove `ANTHROPIC_BASE_URL` from your repo's `.claude/settings.json`!

1. 用 `Command (⌘) + F` 搜索 `TODO`。搜不到结果就说明你完成了。
1. 确认你引用的片段中没有混入凭证或 `x-api-key` 请求头。
1. 将所有改动推送到你的远程仓库，并通过 Gradescope 提交。
